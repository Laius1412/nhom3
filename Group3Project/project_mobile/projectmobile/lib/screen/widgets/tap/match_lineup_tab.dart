...
import '/api/live_score/lineups_api.dart';

class MatchLineupTab extends StatelessWidget {
  final int fixtureId;

  MatchLineupTab({required this.fixtureId});

  @override
  Widget build(BuildContext context) {
    return FutureBuilder<List<Lineup>?>(
      future: LineupAPI.fetchMatchLineup(fixtureId), // Gọi API đúng cách
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return Center(
              child: Text('Không có dữ liệu đội hình', style: TextStyle(color: Colors.white)));
        }

        List<Lineup> lineups = snapshot.data!;
        return SingleChildScrollView(
          child: Column(
            children: lineups.map((lineup) => buildTeamSection(lineup)).toList(),
          ),
        );
      },
    );
